...
export class RDFSResource {
  ...
  public constructor(
    service: RdfService,
    uri?: string,
    type: string = "http://www.w3.org/2000/01/rdf-schema#Resource",
    statement?: TypedNodeQuerySolution
  ) {
    this.uri = "";
    this.types = [];
    ...
    if (statement) {
      if (statement.uri.value in this.service.nodes) {
        ...
        if (
          statement._type &&
          !existingItem.types.includes(statement._type.value)
        ) {
      ...
      if (uri) {
        ...
        if (uri in this.service.nodes) {
          ...
          if (existingItem.constructor != this.constructor) {
            throw Error(
              `Attempt to create a different type of object with uri ${uri}, existing: ${existingItem.constructor}, new: ${this.constructor}`
            );
          }
...
export class RdfService {
  ...
  async runQuery<T>(query: string): Promise<QueryResponse<T>> {
    ...
    if (!response.ok) {
      throw new Error(`runQuery: !response.ok — ${response.statusText}`);
    }

    let results: QueryResponse<T>;
    try {
      results = await response.json();
      return results;
    } catch (jsonError) {
      console.error(`runQuery: json() Failed ${jsonError}`);
      throw jsonError;
    }
    throw new Error("runQuery: Unreachable code was reached");
  }
  ...
  async runUpdate(
    ...
    if (this.#writeEnabled) {
      ...
      if (!response.ok) {
        throw new Error(`runQuery: ${response.statusText}`);
      }
